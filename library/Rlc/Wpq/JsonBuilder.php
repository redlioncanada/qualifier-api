...
class JsonBuilder {
   ...
   * 
   * multidimensional:
   * brand => group identifier => name for API response
   * 
  ...
  private $applianceGroups = [
    'maytag' => [
      'SC_Kitchen_Cooking' => 'Cooking',
      'SC_Laundry_Laundry_Appliances_Laundry_Pairs' => 'Laundry',
      'SC_Kitchen_Dishwashers_and_Kitchen_Cleaning_Dishwashers' => 'Dishwashers',
      'SC_Kitchen_Refrigeration_Refrigerators' => 'Fridges',
    ],
    'whirlpool' => [
      'SC_Kitchen_Dishwasher__Cleaning_Dishwashers' => 'Dishwashers',
    ],
    'kitchenaid' => [
      'SC_Major_Appliances_Dishwashers_Dishwashers' => 'Dishwashers',
    ],
  ];
  ...
  private $typeGroups = [
    'maytag' => [
      'SC_Kitchen_Cooking_Ranges' => 'Ranges',
      'SC_Kitchen_Cooking_Wall_Ovens' => 'Ovens',
    ],
    'whirlpool' => [
    ],
    'kitchenaid' => [
   ...
   * @var array
   * 
   * multidimensional: brand => group identifier
   */
  ...
  public function build($brand, $locale) {
    if (!isset($this->feedModelCache[$brand])) {
      $this->feedModelCache[$brand] = $this->feedModelBuilder->buildFeedModel($brand, $this->includeOnlyGroups[$brand]);
    }
    ...
    foreach ($entries as $entry) {
      ...
      $this->attachGroupData($newOutputData, $entry, $locale, $brand);
  ...
  private function attachGroupData(array &$data, FeedEntity\CatalogEntry $entry,
    ...
    foreach ($allCatalogGroups as $catalogGroup) {
      ...
      if (isset($this->applianceGroups[$groupId])) {
        $data['appliance'] = $this->applianceGroups[$groupId];
      }
      if (isset($this->typeGroups[$brand][$groupId])) {
        $data['type'] = $this->typeGroups[$brand][$groupId];
      }
    }
  }
  ...
  private function attachFeatureData(array &$data,
    ...
    switch ($data['appliance']) {
      case $this->applianceGroups[$brand]['SC_Kitchen_Cooking']: // arr, but the ids are different anyway. need a smarter way.
        switch ($data['type']) {
